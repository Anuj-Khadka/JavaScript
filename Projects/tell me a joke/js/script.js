...
let nextBtn = document.getElementById("navigate-front");
let jokeCard = document.getElementById("joke-card");

let jokeFilter = () => {
  let random_pick = Math.floor(Math.random() * jokeArr.length + 1);
  jokeCard.innerHTML = `
        <p id="joke">Q. "${jokeArr[random_pick][0]}"
        <br><br>A. “${jokeArr[random_pick][1]}”</p>
        `;
}

window.addEventListener("load", jokeFilter);
nextBtn.addEventListener("click", jokeFilter);
